...
async def get_permission_structure(
    ...
    """
    try:
        result = await db.fetch_all_async(permission_query("PERMISSION_STRUCTURE_QUERY"))
...
async def create_users(
    ...
    try:
        ...
        operation_result = await user_service.bulk_create_users(
            org_id=org_id,
            users_data=users_data,
            created_by=current_user.user_id,
        )

        updated_users = await user_service.get_organization_users(
            current_user_id=current_user.user_id,
            offset=offset,
            limit=limit,
        )
...
async def bulk_update_users(
    ...
    try:
        ...
        operation_result = await user_service.bulk_update_users(
            org_id=org_id,
            users_data=users_data,
...
async def bulk_delete_users(
    ...
    try:
        ...
        validated_user_ids: List[int] = []
        for i, user_id in enumerate(user_ids):
            try:
                validated_user_ids.append(int(user_id))
        ...
        operation_result = await user_service.bulk_delete_users(
            org_id=org_id,
    ...
    try:
        ...
        result = {
            ...
            "operation_metadata": {
                ...
                "count": len(paginated_roles.get("items", [])) if isinstance(paginated_roles, dict) else len(paginated_roles),
                "ids": [],
            },
        }
...
async def update_organization_roles(
    ...
    try:
        ...
        operation_result = await role_service.bulk_update_roles(
            org_id=org_id,
...
async def bulk_create_organization_roles(
    ...
    try:
        ...
        for i, role_data in enumerate(roles_data):
            ...
            duplicate_check = [
                r.get("display_name", "").strip() == display_name_clean
        ...
        operation_result = await role_service.bulk_create_roles(
            org_id=org_id,
            roles_data=roles_data,
            created_by=current_user.user_id,
        )
        ...
        logger.info(
            f"User {current_user.user_id} bulk created {operation_result['count']} roles. "
            f"Created IDs: {operation_result['ids']}"
        )

        paginated_result = await db.fetch_one_async(
        ...
        logger.warning(
            f"Bulk create failed for user {current_user.user_id}: {e.message}"
        )
        raise
    except Exception as e:
        logger.exception(
            f"Unexpected error in bulk role creation for user {current_user.user_id}: {e}"
        )
        raise AppException(
...
async def bulk_delete_organization_roles(
    ...
    try:
        ...
        validated_role_ids: List[int] = []
        for i, role_id in enumerate(role_ids):
            try:
                validated_role_ids.append(int(role_id))
            except (ValueError, TypeError):
                raise AppException(
        ...
        operation_result = await role_service.bulk_delete_roles(
            org_id=org_id,
        ...
        logger.warning(
            f"Bulk delete failed for user {current_user.user_id}: {e.message}"
        )
        raise
    except Exception as e:
        logger.exception(
            f"Unexpected error in bulk role deletion: {e}"
        )
        raise AppException(
...
async def get_quick_actions(
    ...
    try:
        perm_system = ExplicitPermissionSystem()
        user_permissions = perm_system.get_user_permission_ids_with_roles(
            current_user.id, db
        )
