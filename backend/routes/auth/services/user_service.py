            ...
            for i, user_data in enumerate(users_data):
                try:
                    ...
                    new_user = self.db.execute_returning(
                        """
                        INSERT INTO users (
                            uid, email, display_name, org_id, department, location,
                            status, status_effective_from, status_effective_to,
                            email_verified, created_at, updated_at
                        ) VALUES (
                            %(uid)s, %(email)s, %(display_name)s, %(org_id)s, %(department)s, %(location)s,
                            %(status)s, %(status_effective_from)s, %(status_effective_to)s,
