...
async def health_check(db = Depends(get_db)):
    ...
    try:
        ...
        db_status = await db.fetch_one_async("SELECT 1 as status", fetch=True) is not None
        ...
        for table in tables_to_check:
            try:
                result = await db.fetch_one_async(f"SELECT COUNT(*) as count FROM {table}", fetch=True)
        ...
        metrics = {}
        try:
            user_count = await db.fetch_one_async("SELECT COUNT(*) as count FROM users", fetch=True)
            permission_count = await db.fetch_one_async("SELECT COUNT(*) as count FROM permissions", fetch=True)
            role_count = await db.fetch_one_async("SELECT COUNT(DISTINCT role_id) as count FROM role_permissions", fetch=True)
