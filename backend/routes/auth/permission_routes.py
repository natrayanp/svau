...
from backend.utils.auth.permissions import (
    ...
    PERMISSION_STRUCTURE, CommonPermissionIds
)
...
# USER PERMISSION MANAGEMENT - ID-BASED ONLY
...
async def grant_permissions(
    ...
    for permission_id in permission_data.permission_ids:
        try:
            ...
            if not perm_system.validate_permission_id(permission_id):
                failed += 1
                continue
...
async def get_all_roles():
    ...
    for role in roles:
        permission_ids = RolePermissions.get_permission_ids_for_role(role)
        role_data.append({
            "name": role,
