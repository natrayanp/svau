<script lang="ts">
  ...
  import type { 
    ...
    CardDetail,
    AllowedAction
  } from '$lib/types_permission';

  import type {
  RolePermissions
  } from '$lib/stroes/types_permission'

  // Props
  export let isViewMode = false;
  export let selectedPermissions: Array<RolePermissions> = [];
  export let onPermissionsChange: (permissions: Array<{
  ...
  $: {
    if (!hasInitialized && selectedPermissions.length > 0) {
  ...
  function isActionGranted(structureId: string, actionKey: string): boolean {
    const structure = localSelectedPermissions.find(item => item.permissstruct_id === structureId);
  ...
  function getAllActionKeysForStructure(structure: ModuleDetail | MenuDetail | CardDetail): string[] {
  ...
  function handleModuleSelection(module: ModuleDetail, event: Event) {
    ...
    const allSelected = allActionKeys.length > 0 && 
                       allActionKeys.every(key => currentActions.includes(key));
    
    if (allSelected) {
      // Deselect all
      updateStructurePermission(module.id, []);
    } else {
      // Select all
      updateStructurePermission(module.id, allActionKeys);
    }
  }
  ...
  function handleMenuSelection(menu: MenuDetail, event: Event) {
    ...
    const allActionKeys = getAllActionKeysForStructure(menu);
    const currentActions = getGrantedActions(menu.id);
    const allSelected = allActionKeys.length > 0 && 
                       allActionKeys.every(key => currentActions.includes(key));
    
    if (allSelected) {
      // Deselect all
      updateStructurePermission(menu.id, []);
    } else {
      // Select all
      updateStructurePermission(menu.id, allActionKeys);
    }
  }
  ...
  function handleCardSelection(card: CardDetail, event: Event) {
    ...
    if (allSelected) {
      // Deselect all
      updateStructurePermission(card.id, []);
    } else {
      // Select all
      updateStructurePermission(card.id, allActionKeys);
    }
  }
  ...
  function handleActionSelection(structureId: string, actionKey: string, event: Event) {
    ...
    const newActions = currentActions.includes(actionKey)
      ? currentActions.filter(key => key !== actionKey) // Remove
      : [...currentActions, actionKey]; // Add
  ...
  function isModulePartiallySelected(module: ModuleDetail): boolean {
    const allActionKeys = getAllActionKeysForStructure(module);
    const currentActions = getGrantedActions(module.id);
  ...
  function isMenuFullySelected(menu: MenuDetail): boolean {
    const allActionKeys = getAllActionKeysForStructure(menu);
    const currentActions = getGrantedActions(menu.id);
    return allActionKeys.length > 0 && 
           allActionKeys.every(key => currentActions.includes(key));
  ...
  function isMenuPartiallySelected(menu: MenuDetail): boolean {
    const allActionKeys = getAllActionKeysForStructure(menu);
    const currentActions = getGrantedActions(menu.id);
    if (allActionKeys.length === 0) return false;
  ...
  function isCardFullySelected(card: CardDetail): boolean {
    const allActionKeys = getAllActionKeysForStructure(card);
    const currentActions = getGrantedActions(card.id);
    return allActionKeys.length > 0 && 
           allActionKeys.every(key => currentActions.includes(key));
  ...
  function isCardPartiallySelected(card: CardDetail): boolean {
    const allActionKeys = getAllActionKeysForStructure(card);
    const currentActions = getGrantedActions(card.id);
    if (allActionKeys.length === 0) return false;
  ...
  function getPermissionTypeColor(type: string): string {
    switch(type) {
      case 'Module': return 'bg-purple-100 text-purple-800 border-purple-200';
      case 'Menu': return 'bg-blue-100 text-blue-800 border-blue-200';
      case 'Card': return 'bg-green-100 text-green-800 border-green-200';
...
<div class="permission-tree bg-white rounded-lg shadow-sm {isViewMode ? 'opacity-95' : ''}">
  ...
  <div class="search-section bg-gray-50 border border-gray-200 rounded-lg p-4 mb-4">
    ...
    <div class="search-actions flex flex-wrap gap-2">
      <button on:click={expandAllNodes} class="btn bg-white border border-gray-300 rounded px-3 py-1 text-sm hover:bg-gray-50 transition-colors">Expand All</button>
      <button on:click={collapseAllNodes} class="btn bg-white border border-gray-300 rounded px-3 py-1 text-sm hover:bg-gray-50 transition-colors">Collapse All</button>
      {#if !isViewMode}
        <button on:click={selectAllPermissions} class="btn bg-white border border-gray-300 rounded px-3 py-1 text-sm hover:bg-gray-50 transition-colors">Select All</button>
        <button on:click={clearAllPermissions} class="btn bg-white border border-gray-300 rounded px-3 py-1 text-sm hover:bg-gray-50 transition-colors">Clear All</button>
      {/if}
    </div>
  </div>
  ...
  <div class="tree-container border border-gray-200 rounded-lg bg-white p-4">
    {#if loading}
      <div class="loading-message text-gray-500 text-center py-5 italic">Loading permission structure...</div>
    {:else if permissionData}
      ...
      <div class="summary-stats mb-4 p-3 bg-gray-50 rounded-lg">
        <div class="flex justify-between items-center">
          <div>
            <span class="text-sm text-gray-600">Selected: <strong>{localSelectedPermissions.reduce((total, perm) => total + perm.granted_action_key.length, 0)}</strong> actions across <strong>{localSelectedPermissions.length}</strong> structures</span>
            <div class="text-xs text-gray-500 mt-1">
              Modules: {permissionData.modules?.length || 0} | 
              Menus: {permissionData.modules?.reduce((total, m) => total + (m.menus?.length || 0), 0)} | 
              Cards: {permissionData.modules?.reduce((total, m) => total + (m.menus?.reduce((menuTotal, menu) => menuTotal + (menu.cards?.length || 0), 0)), 0)}
            </div>
          </div>
          {#if !isViewMode}
            <button on:click={clearAllPermissions} class="text-sm text-red-600 hover:text-red-800">Clear Selection</button>
          {/if}
        </div>
      </div>
        ...
        {#each permissionData.modules as module (module.id)}
          {#if moduleHasMatches(module, searchTerm)}
            ...
            <div class="module-wrapper relative">
              <div class="count-badge absolute -top-2 -left-2 bg-purple-600 text-white rounded-full px-2 py-1 text-xs font-bold shadow-lg border-2 border-white z-10">
                {module.allowed_actions?.length || 0} actions
              </div>
              <div class="module-container bg-purple-50 border-2 border-purple-500 rounded-lg shadow-sm {isViewMode ? '' : 'hover:shadow-md transition-all duration-300 hover:-translate-y-1'} h-fit ml-2 mt-2">
                <!-- Module Header -->
                <div role="button" tabindex="0"
                  class="node-header module-header bg-purple-600 text-white px-4 py-3 rounded-t-lg cursor-pointer select-none {isViewMode ? 'hover:bg-purple-600' : 'hover:bg-purple-700'} transition-colors flex items-center gap-3"
                  on:click={() => toggleNode(module.id)}
                  on:keydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleNode(module.id); } }}
                >
                  <input type="checkbox"
                    class="h-4 w-4 {isViewMode ? 'cursor-default opacity-70' : 'cursor-pointer'}"
                    checked={isModuleFullySelected(module)}
                    indeterminate={isModulePartiallySelected(module) && !isModuleFullySelected(module)}
                  ...
                  <span class="description text-purple-100 text-sm flex-1"> - {module.description}</span>
                  <span class="expand-icon text-xs opacity-90">{expandedNodes.has(module.id) ? '▼' : '▶'}</span>
                </div>
                ...
                {#if expandedNodes.has(module.id)}
                  <div class="module-children p-4 bg-white/50 rounded-b-lg space-y-3">
                        ...
                        <div class="space-y-2 pl-4">
                          {#each module.allowed_actions as action (action.action_key)}
                            <div class="action-item flex items-center gap-2">
                              <input type="checkbox"
                                class="h-4 w-4 {isViewMode ? 'cursor-default opacity-70' : 'cursor-pointer'}"
                                checked={isActionGranted(module.id, action.action_key)}
                                on:click={(e) => handleActionSelection(module.id, action.action_key, e)}
                                on:change={() => {}}
                                disabled={isViewMode}
                              />
                              <span class="name text-sm">{action.display_name}</span>
                              <span class="text-xs px-2 py-0.5 rounded-full bg-purple-100 text-purple-800 border border-purple-200">Module</span>
                              <span class="description text-gray-500 text-xs flex-1"> - {module.name}: {action.display_name}</span>
                            </div>
                          {/each}
                        </div>
                      </div>
                    ...
                    {#each module.menus as menu (menu.id)}
                      {#if menuHasMatches(menu, searchTerm)}
                        <!-- MENU -->
                        <div class="menu-container bg-blue-50 border-2 border-blue-500 rounded-lg shadow-sm {isViewMode ? '' : 'transition-all duration-200'}">
                          <button type="button"
                            class="node-header menu-header bg-blue-600 text-white px-4 py-3 rounded-t-lg cursor-pointer select-none {isViewMode ? 'hover:bg-blue-600' : 'hover:bg-blue-700'} transition-colors flex items-center gap-3 w-full text-left"
                          ...
                          >
                            <input type="checkbox"
                              ...
                              indeterminate={isMenuPartiallySelected(menu) && !isMenuFullySelected(menu)}
                            ...
                            <span class="description text-blue-100 text-sm flex-1 min-w-0"> - {menu.description}</span>
                            <span class="expand-icon text-xs opacity-90">{expandedNodes.has(menu.id) ? '▼' : '▶'}</span>
                          </button>
                            ...
                            <div class="menu-children p-3 bg-white/60 rounded-b-lg space-y-2">
                              ...
                              {#if menu.allowed_actions && menu.allowed_actions.length > 0}
                                <div class="menu-actions-section mb-3">
                                  <h5 class="text-xs font-semibold text-blue-700 mb-1">Menu Actions</h5>
                                  <div class="space-y-1 pl-4">
                                    {#each menu.allowed_actions as action (action.action_key)}
                                      <div class="action-item flex items-center gap-2">
                                        <input type="checkbox"
                                          class="h-3 w-3 {isViewMode ? 'cursor-default opacity-70' : 'cursor-pointer'}"
                                          checked={isActionGranted(menu.id, action.action_key)}
                                          on:click={(e) => handleActionSelection(menu.id, action.action_key, e)}
                                          on:change={() => {}}
                                          disabled={isViewMode}
                                        />
                                        <span class="name text-xs">{action.display_name}</span>
                                        <span class="text-xs px-1.5 py-0.5 rounded-full bg-blue-100 text-blue-800 border border-blue-200">Menu</span>
                                      </div>
                                    {/each}
                                  </div>
                                </div>
                              ...
                              {#each menu.cards as card (card.id)}
                                {#if cardHasMatches(card, searchTerm)}
                                  <!-- CARD -->
                                  <div class="card-container bg-green-50 border-2 border-green-500 rounded-lg shadow-sm {isViewMode ? '' : 'transition-all duration-200'}">
                                    <button type="button"
                                      class="node-header card-header bg-green-500 text-gray-900 px-4 py-2 rounded-t-lg cursor-pointer select-none {isViewMode ? 'hover:bg-green-500' : 'hover:bg-green-600'} transition-colors flex items-center gap-3 w-full text-left"
                                    ...
                                    >
                                      <input type="checkbox"
                                        ...
                                        indeterminate={isCardPartiallySelected(card) && !isCardFullySelected(card)}
                                      ...
                                      <span class="expand-icon text-xs opacity-90">{expandedNodes.has(card.id) ? '▼' : '▶'}</span>
                                    </button>

                                    {#if expandedNodes.has(card.id)}
                                      <div class="card-children p-3 bg-white rounded-b-lg space-y-1">
                                        <!-- Card-level actions -->
                                        {#if card.allowed_actions && card.allowed_actions.length > 0}
                                          <div class="card-actions-section mb-2">
                                            <h6 class="text-xs font-semibold text-green-700 mb-1">Card Actions</h6>
                                            <div class="space-y-1 pl-4">
                                              {#each card.allowed_actions as action (action.action_key)}
                                                <div class="action-item flex items-center gap-2">
                                                  <input type="checkbox"
                                                    class="h-3 w-3 {isViewMode ? 'cursor-default opacity-70' : 'cursor-pointer'}"
                                                    checked={isActionGranted(card.id, action.action_key)}
                                                    on:click={(e) => handleActionSelection(card.id, action.action_key, e)}
                                                    on:change={() => {}}
                                                    disabled={isViewMode}
                                                  />
                                                  <span class="name text-xs">{action.display_name}</span>
                                                  <span class="text-xs px-1.5 py-0.5 rounded-full bg-green-100 text-green-800 border border-green-200">Card</span>
                                                </div>
                                              {/each}
                                            </div>
                                          </div>
