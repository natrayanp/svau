...
*/
			...
			roleData = {
				...
				power_level:
					$selectedPermissionIds.length > 0
						? PermissionUtils.getMaxPower($selectedPermissionIds, $permissionStructure)
						: 10,
  ...
  function initializeSelections() {
	...
	function isModulePartiallySelected(module) {
		const allPermissions = getAllPermissionsInModule(module);
		const selectedCount = allPermissions.filter((perm) =>
	...
	function isCardPartiallySelected(card) {
		const selectedCount = (card.permissions || []).filter((perm) =>
			$selectedPermissionIds.includes(perm.id)
		).length;
	...
	function getFilteredModules() {
		...
		return $permissionStructure.modules
			.map((module) => ({
				...module,
				menus: module.menus
					.map((menu) => ({
						...
						cards: menu.cards
							.map((card) => ({
								...card,
								permissions: (card.permissions || []).filter(
									(perm) =>
										!$treeSelection.searchTerm ||
										perm.display_name
											.toLowerCase()
											.includes($treeSelection.searchTerm.toLowerCase()) ||
					...
					.filter(
						(menu) => menu.cards.length > 0 || (menu.permissions && menu.permissions.length > 0)
					)
			}))
		...
		{:else}
					...
					<div class="flex items-center justify-between">
						<div class="flex items-center space-x-6">
							<a
								...
								class="group flex items-center space-x-2 text-slate-600 hover:text-indigo-600 transition-all duration-300"
							>
								<div
									class="w-8 h-8 bg-white/80 border border-slate-200 rounded-lg flex items-center justify-center group-hover:border-indigo-300 group-hover:bg-indigo-50 transition-all duration-300"
								>
							...
							<div class="flex items-center space-x-3">
								<div class="relative">
									...
									>
										<span class="text-white font-bold text-lg"
											>{PermissionUtils.getPowerLevelIcon(roleData?.power_level || 0)}</span
										>
									</div>
									<div
								...
								<div>
									<div class="font-semibold text-slate-900 text-lg">
										{PermissionUtils.getPowerLevelLabel(roleData?.power_level || 0)}
									</div>
			...
			<div class="mb-8">
				<div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 p-2">
					<nav class="flex space-x-2">
						<button
							class="flex-1 flex items-center justify-center space-x-3 py-4 px-6 rounded-xl font-semibold text-lg transition-all duration-300 {activeTab ===
							'permissions'
			...
			{#if activeTab === 'permissions'}
				...
				>
					<!-- Premium Toolbar -->
					<div
						class="border-b border-slate-200/60 p-6 bg-gradient-to-r from-slate-50 to-blue-50/30"
					>
						<div class="flex items-center justify-between">
							<div class="flex items-center space-x-4">
								...
								<div class="relative group">
									<div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
										<span
											class="text-slate-400 group-focus-within:text-indigo-500 transition-colors duration-300"
											>🔍</span
										>
									</div>
									<input
							...
							<div class="text-right">
								...
								<div
									class="font-bold text-2xl bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent"
								>
									{$selectedPermissionIds?.length || 0}
									...
									{#each module.menus as menu}
										...
										<div class="premium-menu-card ml-8 mt-4">
											...
											<div class="flex items-center space-x-4 p-6 border-b border-slate-200/60">
												...
												<div
													class="w-10 h-10 bg-gradient-to-br from-slate-500 to-slate-600 rounded-lg flex items-center justify-center shadow-md"
												>
											...
											{#if menu.permissions && menu.permissions.length > 0}
												<div class="p-6">
													<div
														class="equal-width-permissions-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"
													>
															...
															<div class="premium-permission-card group">
																<div class="flex items-center space-x-4 p-4">
																	<input
																		...
																		on:change={() =>
																			permissionActions.togglePermissionGlobally(permission.id)}
																	...
																	<div class="flex-1 min-w-0">
																		...
																		<div class="flex items-center justify-between mt-2">
																			...
																			<div class="flex items-center space-x-2">
																				<div
																					class="w-2 h-2 bg-gradient-to-r {getPowerLevelColor(
																						permission.power_level
																					)} rounded-full"
																				></div>
																				<span
																					class="text-xs font-bold {getPowerLevelBg(
																						permission.power_level
												...
												{#each menu.cards as card}
													<div class="premium-card-card ml-6 mt-4">
														<div class="flex items-center space-x-4 p-5">
															...
															<div
																class="w-9 h-9 bg-gradient-to-br from-slate-400 to-slate-500 rounded-lg flex items-center justify-center shadow-sm"
															>
																		...
																		<div class="premium-permission-card group">
																			<div class="flex items-center space-x-4 p-4">
																				<input
																					...
																					checked={isPermissionSelected(permission.id)}
																					on:change={() =>
																						permissionActions.togglePermissionGlobally(
																							permission.id
																						)}
																					class="premium-checkbox h-5 w-5 text-indigo-600 focus:ring-indigo-500 border-slate-300 rounded-lg flex-shrink-0"
																				/>
																				...
																				<div class="flex-1 min-w-0">
																					<div
																						class="font-semibold text-slate-900 text-sm truncate"
																					>
																						{permission.display_name}
																					</div>
																					<div class="flex items-center justify-between mt-2">
					...
					<div
						class="border-t border-slate-200/60 p-6 bg-gradient-to-r from-slate-50 to-blue-50/30"
					>
						<div class="flex items-center justify-between">
							<div class="flex items-center space-x-6">
									...
									<div class="flex items-center space-x-4">
										<div class="flex items-center space-x-2">
											...
											<span class="text-sm font-medium text-slate-700"
												>{$selectedPermissionsAnalysis.power_distribution.low} Low</span
											>
										</div>
										<div class="flex items-center space-x-2">
											<div class="w-3 h-3 bg-amber-500 rounded-full"></div>
											<span class="text-sm font-medium text-slate-700"
												>{$selectedPermissionsAnalysis.power_distribution.medium} Medium</span
											>
										</div>
										<div class="flex items-center space-x-2">
											<div class="w-3 h-3 bg-orange-500 rounded-full"></div>
											<span class="text-sm font-medium text-slate-700"
												>{$selectedPermissionsAnalysis.power_distribution.high} High</span
											>
										</div>
										<div class="flex items-center space-x-2">
											<div class="w-3 h-3 bg-rose-500 rounded-full"></div>
											<span class="text-sm font-medium text-slate-700"
												>{$selectedPermissionsAnalysis.power_distribution.critical} Critical</span
											>
										</div>
									</div>
								{:else}
							...
							<div class="flex items-center space-x-4">
								{#if saveMessage}
									<div
										class="flex items-center space-x-2 px-4 py-2 rounded-lg {saveMessage.includes(
											'✅'
										)
			...
			{#if activeTab === 'users'}
				<div
					class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 p-12"
				>
					<div class="text-center py-12">
						<div
							class="w-24 h-24 bg-gradient-to-br from-slate-100 to-slate-200 rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-lg"
						>
						...
						<p class="text-slate-600 text-lg mb-8 max-w-md mx-auto">
							Manage users assigned to the <span class="font-semibold text-slate-900"
								>{roleData?.name}</span
							> role
						</p>
						<button
			...
			{#if activeTab === 'analysis'}
				...
				>
					<div class="text-center py-12">
						...
						<p class="text-slate-600 text-lg mb-8 max-w-md mx-auto">
							Detailed power analysis for the <span class="font-semibold text-slate-900"
								>{roleData?.name}</span
							> role
						</p>
						<button
